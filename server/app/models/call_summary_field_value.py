"""
CallSummaryFieldValue model - per-call-record values for call summary forms.
"""
...
class CallSummaryFieldValue(Base, TimestampMixin):
    __tablename__ = "call_summary_field_values"
    __table_args__ = (
        Index("ix_call_summary_field_values_tenant_call_record", "tenant_id", "call_record_id"),
        Index(
    ...
    call_record_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("call_records.id", ondelete="CASCADE"), nullable=False
    )
    ...
    field_definition: Mapped["FdFieldDefinition | None"] = relationship(
        "FdFieldDefinition", lazy="selectin"
    )
