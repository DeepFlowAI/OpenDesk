"""
Pydantic schemas for user view management
"""
...
class UserViewResponse(UserViewBase, TimestampSchema):
...
class UserViewListResponse(PaginatedResponse):
    items: list[UserViewResponse]


class UserViewToggleRequest(BaseModel):
    is_enabled: bool


class UserViewSortItem(BaseModel):
    id: int
    sort_order: int


class UserViewSortRequest(BaseModel):
    items: list[UserViewSortItem]
