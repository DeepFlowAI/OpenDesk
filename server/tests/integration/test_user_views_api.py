"""
Integration tests for UserView API
"""
import uuid
...
class TestUserViewsAPI:

    @pytest.mark.asyncio
    async def test_list_returns_200(self, client: AsyncClient):
        headers = _auth_header()
        resp = await client.get("/api/v1/user-views", headers=headers)
    ...
    async def test_create_returns_201(self, client: AsyncClient):
        headers = _auth_header()
        payload = _view_payload("高价值客户")
        resp = await client.post("/api/v1/user-views", headers=headers, json=payload)
        assert resp.status_code == 201
        data = resp.json()
        assert data["name"] == "高价值客户"
    ...
    async def test_create_missing_name_returns_422(self, client: AsyncClient):
        headers = _auth_header()
        resp = await client.post("/api/v1/user-views", headers=headers, json={})
    ...
    async def test_get_by_id_returns_200(self, client: AsyncClient):
        ...
        resp = await client.get(f"/api/v1/user-views/{view_id}", headers=headers)
        assert resp.status_code == 200
    ...
    async def test_get_not_found_returns_404(self, client: AsyncClient):
        headers = _auth_header()
        resp = await client.get("/api/v1/user-views/99999", headers=headers)
    ...
    async def test_delete_returns_200(self, client: AsyncClient):
        ...
        resp = await client.delete(f"/api/v1/user-views/{view_id}", headers=headers)
        assert resp.status_code == 200

        get_resp = await client.get(f"/api/v1/user-views/{view_id}", headers=headers)
    ...
    async def test_toggle_enabled(self, client: AsyncClient):
        ...
        resp = await client.put(
            f"/api/v1/user-views/{view_id}/toggle",
            headers=headers,
    ...
    async def test_create_with_conditions(self, client: AsyncClient):
        ...
        payload["conditions"] = [
            ...
            {"field_id": 2, "operator": "is_not_empty", "value": None},
        ]
    ...
    async def test_create_with_columns_config(self, client: AsyncClient):
        ...
        payload["columns_config"] = [
            ...
            {"field_id": 2, "visible": False, "sort_order": 1},
        ]
        resp = await client.post("/api/v1/user-views", headers=headers, json=payload)
        assert resp.status_code == 201
        data = resp.json()
