...
async def test_get_settings_returns_empty_state_for_new_tenant(client: AsyncClient):
    tenant_id = await _ensure_tenant()

    resp = await client.get("/api/v1/open-agent-settings", headers=_auth_header(tenant_id))
...
async def test_list_agents_uses_saved_secret(client: AsyncClient, monkeypatch):
    ...
    resp = await client.get("/api/v1/open-agent-settings/agents", headers=_auth_header(tenant_id))
