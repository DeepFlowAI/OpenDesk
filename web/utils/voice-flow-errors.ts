...
const FIELD_LABELS: Array<{ test: RegExp; label: string }> = [
  ...
  { test: /^target_strategy$/,               label: '多队列选择方式' },
  { test: /^queue_targets$/,                 label: '候选队列' },
  { test: /^queue_targets\.(\d+)\.queue_type$/, label: '候选队列 · 类型' },
  { test: /^queue_targets\.(\d+)\.queue_id$/,   label: '候选队列 · 目标' },
  { test: /^queue_prompt_text$/,             label: '排队提示话术' },
  { test: /^prompt_play_mode$/,              label: '播放方式' },
  { test: /^prompt_loop_interval_seconds$/,  label: '循环间隔' },
  { test: /^queue_limit_prompt_text$/,       label: '达到排队上限提示' },
  { test: /^no_available_queue_prompt_text$/, label: '无可用队列提示' },
  { test: /^queue_timeout_prompt_text$/,     label: '排队超时提示' },
  { test: /^agent_no_answer_prompt_text$/,   label: '坐席未接听提示' },
...
const CUSTOM_CODE_MSG: Record<string, string> = {
  ...
  queue_not_found: '引用的队列目标不存在或不可用',
  queue_field_not_found: '引用的用户字段不存在或不可用于队列路由',
