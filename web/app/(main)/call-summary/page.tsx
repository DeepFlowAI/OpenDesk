...
import {
  useCallSummaryFields,
  useAddCallSummaryField,
  useDeleteCallSummaryField,
  useSortCallSummaryFields,
  useUpdateCallSummaryField,
  useCallSummaryRules,
  useCreateCallSummaryRule,
  useUpdateCallSummaryRule,
  useDeleteCallSummaryRule,
  useSortCallSummaryRules,
} from '@/service/use-call-summary'
import { useFieldDefinitions } from '@/service/use-field-definitions'
import type { CallSummaryConfigField, CallSummaryInteractionRule } from '@/models/call-summary'
...
export default function CallSummaryPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-semibold text-foreground">{t('callSummary.title', locale)}</h1>
      ...
      <div className="flex gap-4 border-b border-border">
        {(['fields', 'rules'] as const).map((tab) => (
          ...
          >
            {t(tab === 'fields' ? 'callSummary.tab.fieldsSort' : 'callSummary.tab.interactionRules', locale)}
          </button>
        ))}
      </div>
...
function FieldsTab() {
  const { locale } = useLocaleStore()
  const { data, isLoading } = useCallSummaryFields()
  const deleteMutation = useDeleteCallSummaryField()
  const sortMutation = useSortCallSummaryFields()
  const updateMutation = useUpdateCallSummaryField()
  ...
  const [sortedItems, setSortedItems] = useState<CallSummaryConfigField[]>([])
  const [dragIdx, setDragIdx] = useState<number | null>(null)
  const [showPoolModal, setShowPoolModal] = useState(false)
  const [deleteConfirm, setDeleteConfirm] = useState<CallSummaryConfigField | null>(null)

  const fields = data?.items ?? []
  const deleteFieldDef = deleteConfirm
    ? poolData?.items?.find((fd) => fd.id === deleteConfirm.field_definition_id)
    : undefined
  ...
  const poolFields = useMemo(() => {
    ...
    return poolData.items.filter(
      (fd) =>
        fd.applicable_modules?.includes('call_summary') && !addedDefIds.has(fd.id)
    )
  }, [poolData, fields])
  ...
  const toggleActive = (field: CallSummaryConfigField) => {
  ...
  if (isLoading) {
    return <p className="text-sm text-muted-foreground">{t('callSummary.fields.loading', locale)}</p>
  }
    ...
    <>
      ...
      {sorting ? (
        <div className="flex items-center justify-between rounded-lg bg-accent px-4 py-3">
          <span className="text-sm text-muted-foreground">{t('callSummary.fields.sortHint', locale)}</span>
          <div className="flex gap-2">
            <button
              onClick={cancelSort}
              className="rounded-lg border border-border bg-white px-4 py-1.5 text-sm"
            >
              {t('callSummary.fields.sortCancel', locale)}
            </button>
            <button
              onClick={confirmSort}
              className="rounded-lg bg-primary px-4 py-1.5 text-sm text-white"
            >
              {t('callSummary.fields.sortConfirm', locale)}
            </button>
          </div>
        </div>
      ) : (
        <div className="flex items-center gap-3">
          {fields.length > 1 && (
            ...
            >
              ...
              {t('callSummary.fields.sort', locale)}
            </button>
          )}
          <button
          ...
          >
            <IconPlus size={16} />
            {t('callSummary.fields.addFromPool', locale)}
          </button>
        </div>
      )}
      ...
      {displayItems.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-16 text-muted-foreground">
          <p className="text-sm">{t('callSummary.fields.empty', locale)}</p>
          <button
            onClick={() => setShowPoolModal(true)}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-muted-foreground">
              ...
              <th className="w-16 py-2">{t('callSummary.fields.col.index', locale)}</th>
              <th className="py-2">{t('callSummary.fields.col.name', locale)}</th>
              <th className="py-2">{t('callSummary.fields.col.type', locale)}</th>
              <th className="w-20 py-2">{t('callSummary.fields.col.status', locale)}</th>
              {!sorting && <th className="w-24 py-2">{t('callSummary.fields.col.actions', locale)}</th>}
            </tr>
          </thead>
          <tbody>
      ...
      {deleteConfirm && (
        <ConfirmDialog
          title={t('callSummary.fields.delete.title', locale)}
          message={t('callSummary.fields.delete.confirm', locale)}
          details={[
            {
              label: t('callSummary.fields.delete.fieldName', locale),
              value: deleteFieldDef?.name ?? deleteConfirm.field_key ?? '—',
            },
            {
              label: t('callSummary.fields.delete.fieldType', locale),
              value: deleteFieldDef?.field_type
                ? FIELD_TYPE_LABELS[deleteFieldDef.field_type as FieldType]?.[locale as 'zh' | 'en'] ?? deleteFieldDef.field_type
                : '—',
            },
          ]}
          cancelLabel={t('callSummary.fields.delete.cancel', locale)}
          okLabel={t('callSummary.fields.delete.ok', locale)}
...
function PoolModal({
  ...
  const { locale } = useLocaleStore()
  const addMutation = useAddCallSummaryField()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-[520px] rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <h3 className="text-lg font-semibold">{t('callSummary.fields.pool.title', locale)}</h3>
          <button onClick={onClose} className="text-sm text-muted-foreground hover:text-foreground">
            {t('callSummary.fields.pool.close', locale)}
          </button>
        </div>
        <div className="max-h-[400px] overflow-y-auto px-6 py-4">
          {poolFields.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">
              {t('callSummary.fields.pool.empty', locale)}
            </p>
          ) : (
            <div className="flex flex-col gap-2">
              {poolFields.map((fd) => (
                ...
                >
                  ...
                  >
                    {t('callSummary.fields.pool.add', locale)}
                  </button>
                </div>
              ))}
            </div>
...
function RulesTab() {
  const { locale } = useLocaleStore()
  const { data, isLoading } = useCallSummaryRules()
  const deleteMutation = useDeleteCallSummaryRule()
  const updateMutation = useUpdateCallSummaryRule()
  const sortMutation = useSortCallSummaryRules()

  const [sorting, setSorting] = useState(false)
  const [sortedRules, setSortedRules] = useState<CallSummaryInteractionRule[]>([])
  const [dragIdx, setDragIdx] = useState<number | null>(null)
  const [drawerRule, setDrawerRule] = useState<CallSummaryInteractionRule | 'new' | null>(null)
  const [deleteConfirm, setDeleteConfirm] = useState<CallSummaryInteractionRule | null>(null)
  ...
  const toggleEnabled = (rule: CallSummaryInteractionRule) => {
  ...
  if (isLoading) {
    return <p className="text-sm text-muted-foreground">{t('callSummary.rules.loading', locale)}</p>
  }
  ...
  return (
    <>
      ...
      {sorting ? (
        <div className="flex items-center justify-between rounded-lg bg-accent px-4 py-3">
          <span className="text-sm text-muted-foreground">{t('callSummary.rules.sortHint', locale)}</span>
          <div className="flex gap-2">
            <button
              onClick={cancelSort}
              className="rounded-lg border border-border bg-white px-4 py-1.5 text-sm"
            >
              {t('callSummary.rules.sortCancel', locale)}
            </button>
            <button
              onClick={confirmSort}
              className="rounded-lg bg-primary px-4 py-1.5 text-sm text-white"
            >
              {t('callSummary.rules.sortConfirm', locale)}
            </button>
          </div>
        </div>
      ) : (
        <div className="flex items-center gap-3">
          {rules.length > 1 && (
            ...
            >
              <IconArrowsSort size={16} />
              {t('callSummary.rules.sort', locale)}
            </button>
          )}
          <button
          ...
          >
            <IconPlus size={16} />
            {t('callSummary.rules.add', locale)}
          </button>
        </div>
      )}
      ...
      {displayRules.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-16 text-muted-foreground">
          <p className="text-sm">{t('callSummary.rules.empty', locale)}</p>
        </div>
      ) : (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-muted-foreground">
              {sorting && <th className="w-10 py-2" />}
              <th className="w-16 py-2">{t('callSummary.rules.col.index', locale)}</th>
              <th className="py-2">{t('callSummary.rules.col.summary', locale)}</th>
              <th className="w-20 py-2">{t('callSummary.rules.col.enabled', locale)}</th>
              {!sorting && <th className="w-28 py-2">{t('callSummary.rules.col.actions', locale)}</th>}
            </tr>
          </thead>
          <tbody>
      ...
      {deleteConfirm && (
        <ConfirmDialog
          title={t('callSummary.rules.delete.title', locale)}
          message={t('callSummary.rules.delete.confirm', locale)}
          details={[
            {
              label: t('callSummary.rules.delete.ruleName', locale),
              value: deleteConfirm.name ?? '—',
            },
          ]}
          cancelLabel={t('callSummary.rules.delete.cancel', locale)}
          okLabel={t('callSummary.rules.delete.ok', locale)}
          onCancel={() => setDeleteConfirm(null)}
          onConfirm={confirmDelete}
        />
      )}
...
function RuleDrawer({
  ...
  rule: CallSummaryInteractionRule | null
  onClose: () => void
}) {
  const { locale } = useLocaleStore()
  const createMutation = useCreateCallSummaryRule()
  const updateMutation = useUpdateCallSummaryRule()

  const { data: fieldsData } = useCallSummaryFields()
  const { data: poolData } = useFieldDefinitions({
    domain: 'shared_pool',
    status: 'active',
    per_page: 200,
  ...
  const handleSave = async () => {
    const validationError = validateRuleDraft(draft, locale)
    if (validationError) {
  ...
  const stateOptions = [
    { value: 'hidden', label: t('callSummary.rules.state.hidden', locale) },
    { value: 'required', label: t('callSummary.rules.state.required', locale) },
    { value: 'optional', label: t('callSummary.rules.state.optional', locale) },
    { value: 'readonly', label: t('callSummary.rules.state.readonly', locale) },
  ]
        ...
        <div className="flex h-14 items-center justify-between border-b border-border px-5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-foreground">
              {t(rule ? 'callSummary.rules.drawer.editTitle' : 'callSummary.rules.drawer.newTitle', locale)}
            </span>
          </div>
          <button
            onClick={handleSave}
          ...
          >
            {saving ? t('callSummary.rules.drawer.saving', locale) : t('callSummary.rules.drawer.save', locale)}
          </button>
        </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground/80">
                {t('callSummary.rules.form.name', locale)}
              </label>
              <input
                type="text"
                value={draft.name}
                onChange={(e) => updateDraft((d) => ({ ...d, name: e.target.value }))}
                placeholder={t('callSummary.rules.form.name.placeholder', locale)}
            ...
            <div className="flex items-center gap-3">
              <label className="text-sm font-medium text-foreground/80">
                {t('callSummary.rules.form.enabled', locale)}
              </label>
              <button
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground/80">
                {t('callSummary.rules.form.conditionLogic', locale)}
              </label>
              ...
              <div className="mb-4 flex gap-2">
                {(['and', 'or'] as const).map((logic) => (
                  ...
                  >
                    {t(logic === 'and' ? 'callSummary.rules.form.conditionLogic.all' : 'callSummary.rules.form.conditionLogic.any', locale)}
                  </button>
                ))}
              </div>
                ...
                {draft.conditions.map((cond, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      >
                        <option value="">{t('callSummary.rules.form.field.placeholder', locale)}</option>
                      ...
                      >
                        <option value="">{t('callSummary.rules.form.operator.placeholder', locale)}</option>
                        {operators.map((operator) => (
                          <option key={operator} value={operator}>{t(`callSummary.rules.op.${operator}`, locale)}</option>
                        ))}
                      </select>
                      {showValueEditor && (
                        <div className="min-w-[140px] flex-1">
                          <FilterValueEditor
                            ...
                            placeholder={t('callSummary.rules.form.value.placeholder', locale)}
                          />
                        </div>
                      )}
                      <button
              ...
              >
                <IconPlus size={14} />
                {t('callSummary.rules.form.addCondition', locale)}
              </button>
            </div>
            ...
            <div>
              <label className="mb-3 block text-sm font-semibold text-foreground">
                {t('callSummary.rules.form.actions', locale)}
              </label>
                  ...
                  <div key={idx} className="flex flex-col gap-3">
                    <div>
                      <label className="mb-1 block text-sm font-medium text-foreground/80">
                        {t('callSummary.rules.form.targetField', locale)}
                      </label>
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className={cn(selectCls, 'min-w-0 flex-1')}
                        >
                          <option value="">{t('callSummary.rules.form.targetField.placeholder', locale)}</option>
                          {availableFields.map((o) => (
                            <option key={o.value} value={o.value}>{o.label}</option>
                          ))}
                        </select>
                    ...
                    <div>
                      <label className="mb-1 block text-sm font-medium text-foreground/80">
                        {t('callSummary.rules.form.targetState', locale)}
                      </label>
                      <select
                        value={act.state}
                        onChange={(e) =>
                      ...
                      >
                        <option value="">{t('callSummary.rules.form.targetState.placeholder', locale)}</option>
                        {stateOptions.map((o) => (
                          <option key={o.value} value={o.value}>{o.label}</option>
                        ))}
                      </select>
              ...
              >
                <IconPlus size={14} />
                {t('callSummary.rules.form.addAction', locale)}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="w-[400px] rounded-xl bg-white p-6 shadow-xl">
        ...
        {details && details.length > 0 && (
          <div className="mb-6 rounded-lg border border-border bg-muted/40 p-3">
            {details.map((item) => (
              <div key={item.label} className="flex justify-between gap-4 py-1 text-sm">
...
function buildRuleSummary(rule: CallSummaryInteractionRule, locale: string): string {
  ...
  const condPart = conds
    ...
    .map((c) => {
      ...
      const op = t(`callSummary.rules.op.${c.operator}`, lang) || c.operator
  ...
  const actPart = acts
    ...
    .map((a) => {
      ...
      const state = t(`callSummary.rules.state.${a.state}`, lang) || a.state
      return `${target} → ${state}`
    })
    .join(', ')
...
function validateRuleDraft(draft: RuleDraft, locale: string): string | null {
  const lang = locale as 'zh' | 'en'
  if (!draft.name.trim()) return t('callSummary.rules.form.validation.name', lang)
  if (
    draft.conditions.length === 0 ||
    draft.conditions.some((condition) => {
      const hasField = !!condition.field_key || !!condition.field_id
      return !hasField || !condition.operator || !isConditionValueComplete(condition.operator, condition.value)
    })
  ) {
    return t('callSummary.rules.form.validation.conditions', lang)
  }
  if (
    draft.actions.length === 0 ||
    draft.actions.some((action) => {
      const hasField = !!action.target_field_key || !!action.target_field_id
      return !hasField || !action.state
    })
  ) {
    return t('callSummary.rules.form.validation.actions', lang)
  }
  return null
}
