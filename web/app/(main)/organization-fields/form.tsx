...
export default function OrgFieldForm({ initialData, isEdit, onSubmit }: OrgFieldFormProps) {
  ...
  const validate = useCallback((): boolean => {
    ...
    if (!form.name.trim()) {
      errs.name = t('of.form.name.required', locale)
    }
    const key = form.key.trim()
    if (!key) {
      errs.key = t('of.form.key.required', locale)
    } else if (key.length < 2 || key.length > 64) {
      errs.key = t('of.form.key.length', locale)
    } else if (!isFieldKeyValid(key)) {
      errs.key = t('of.form.key.format', locale)
    }
    if (!form.field_type) {
      errs.field_type = t('of.form.fieldType.required', locale)
    }
    const isSelect = (SELECT_FIELD_TYPES as readonly string[]).includes(form.field_type)
    if (isSelect && form.options.length === 0) {
      errs.options = t('of.form.optionsRequired', locale)
    }
    setErrors(errs)
    ...
    (e: React.FormEvent) => {
      ...
      if (isEdit) {
        ...
        const payload: CreateFdFieldDefinitionPayload = {
          domain: FieldDomain.ORGANIZATION,
          key: form.key.trim(),
          name: form.name.trim(),
          description: form.description.trim() || null,
          help_text: form.help_text.trim() || null,
  ...
  return (
    <form id="org-field-form" onSubmit={handleSubmit} className="flex flex-col gap-5 p-8">
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-foreground/80">
          {t('of.form.name', locale)} <span className="text-destructive">*</span>
        </label>
        <input
          type="text"
          value={form.name}
          onChange={(e) => handleNameChange(e.target.value)}
          placeholder={t('of.form.name.placeholder', locale)}
          maxLength={64}
      ...
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-foreground/80">
          {t('of.form.key', locale)} <span className="text-destructive">*</span>
        </label>
        <input
          type="text"
          value={form.key}
          onChange={(e) => handleKeyChange(e.target.value)}
          placeholder={t('of.form.key.placeholder', locale)}
          maxLength={64}
          disabled={isEdit}
          className={cn(inputClass, isEdit && 'bg-accent text-muted-foreground', errors.key && 'border-destructive')}
        />
        <p className="text-xs text-muted-foreground">
          {isEdit ? t('of.form.key.locked', locale) : t('of.form.key.help', locale)}
        </p>
        {errors.key && <p className="text-xs text-destructive">{errors.key}</p>}
      </div>

      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-foreground/80">{t('of.form.description', locale)}</label>
        <textarea
          value={form.description}
          onChange={(e) => set('description', e.target.value)}
          placeholder={t('of.form.description.placeholder', locale)}
          rows={3}
          maxLength={500}
      ...
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-foreground/80">{t('of.form.helpText', locale)}</label>
        <textarea
          value={form.help_text}
          onChange={(e) => set('help_text', e.target.value)}
          placeholder={t('of.form.helpText.placeholder', locale)}
          rows={2}
          maxLength={200}
          className={cn(textareaClass, 'align-top')}
        />
      ...
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-foreground/80">
          {t('of.form.fieldType', locale)} <span className="text-destructive">*</span>
        </label>
        {isEdit ? (
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-xs text-muted-foreground">
              <IconLock size={12} />
              {t('of.form.fieldType.locked', locale)}
            </span>
          </div>
        ) : (
          <div className="relative w-[240px]">
            ...
            >
              <span>{selectedTypeLabel || t('of.form.fieldType.placeholder', locale)}</span>
      ...
      <div className="flex items-center gap-4">
        <label className="text-sm font-medium text-foreground/80">{t('of.form.showInWorkspace', locale)}</label>
        <button
          type="button"
          role="switch"
      ...
      <div className="flex items-center gap-4">
        <label className="text-sm font-medium text-foreground/80">{t('of.form.status', locale)}</label>
