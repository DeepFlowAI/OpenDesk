...
export default function WorkspaceTicketsPage() {
  ...
  const { data: views, isLoading: viewsLoading } = useEnabledTicketViews()
  const { data: viewCountsData } = useTicketViewCounts()
  ...
  const totalTicketCount = viewCountsData?.total_count ?? 0
  ...
  const activeView: TicketView | null = useMemo(() => {
  ...
  const { data: groupsData, isLoading: groupsLoading } = useTicketViewGroups(
  ...
  const queryPayload: TicketQueryPayload = useMemo(
    () => ({
  ...
  const { data: ticketsData, isLoading: ticketsLoading } = useQueryTickets(queryPayload)
  ...
  const handleSelectGroup = useCallback((value: string | undefined) => {
    setGroupValue(value)
    setPage(1)
    router.replace(buildTicketsUrl(selectedViewId, value), { scroll: false })
  ...
  return (
    <div className="flex h-full">
          ...
          <div>
            ...
            >
              <span className="flex items-center gap-2">
                <IconTicket size={18} className="text-foreground" />
      ...
      {/* Right: Main — same chrome as workspace users / org lists (3.1 pen) */}
      <div className="flex flex-1 flex-col overflow-hidden bg-white">
        ...
        <div className="flex items-center gap-3 px-5 pb-3.5 pt-3">
          <div className="relative w-full max-w-[240px]">
            ...
            <input
              ...
              placeholder={locale === 'zh' ? '搜索工单...' : 'Search tickets...'}
              className={cn(
        ...
        {/* Inset rule: full-width border-b would bleed to panel edges; match table horizontal inset */}
          ...
          {ticketsLoading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3">
              <p className="text-sm text-muted-foreground">
                {hasActiveFilter
                  ? (locale === 'zh' ? '无匹配条件的工单' : 'No tickets match the filter')
                  : (locale === 'zh' ? '暂无工单数据' : 'No ticket data')}
              </p>
              {hasActiveFilter && (
                <button
                  onClick={handleClearFilter}
              ...
              <tbody>
                {ticketsData.items.map((ticket) => (
                  <tr
                    ...
                    onClick={() => router.push(`/workspace/tickets/${ticket.id}?from=list`)}
                  >
                    {displayColumns.map((col, idx) => {
                      ...
                      return (
                        <td
                          ...
                          style={{ minWidth: getColumnMinWidth(col.field_type, col.field_key) }}
                        >
                          {isStatusCol && ticket.status ? (
                            <FieldOptionPill
                              label={val}
                              color={colorForSelectValue(fieldByKey.get('status'), ticket.status)}
                            />
                          ) : isPriorityCol && ticket.priority ? (
                            <FieldOptionPill
                              label={val}
                              color={colorForSelectValue(
                                fieldByKey.get('priority'),
                                ticket.priority,
                              )}
                            />
...
function buildTicketsUrl(
  ...
  return qs ? `/workspace/tickets?${qs}` : '/workspace/tickets'
}
...
function getCellValue(
  ...
  locale: string,
  fieldByKey: Map<string, UnifiedField>,
): string {
  ...
  if (field_key && SYSTEM_KEYS.has(field_key)) {
    ...
    if (field_key === 'status') {
      return labelForSelectValue(fieldByKey.get('status'), String(raw))
    }
