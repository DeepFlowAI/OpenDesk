...
export default function WorkspaceOrganizationsPage() {
  ...
  const { data: orgViews, isLoading: orgViewsLoading } = useEnabledOrgViews()
  const { data: orgViewCountsData } = useOrgViewCounts()

  // User views & counts (for sidebar user group)
  const { data: userViews, isLoading: userViewsLoading } = useEnabledUserViews()
  const { data: userViewCountsData } = useUserViewCounts()

  const { data: fieldsData } = useUnifiedFields({ domain: 'organization', include_metadata: true })
  ...
  const totalUserCount = userViewCountsData?.total_count ?? 0
  const userViewCountMap = useMemo(() => {
    const m = new Map<number, number>()
    for (const item of userViewCountsData?.items ?? []) m.set(item.view_id, item.count)
    return m
  }, [userViewCountsData])
  ...
  const activeView: OrganizationView | null = useMemo(() => {
    if (!orgViews || selectedViewId === 'all') return null
    return orgViews.find((v) => v.id === selectedViewId) ?? null
  }, [orgViews, selectedViewId])
  ...
  const { data: groupsData, isLoading: groupsLoading } = useOrgViewGroups(
  ...
  // Build query payload
  const queryPayload: OrganizationQueryPayload = useMemo(
    () => ({
  ...
  // Always show user_count as a pinned column after system/custom fields
  const userCountCol: DisplayColumn = useMemo(() => ({
    field_key: '__user_count',
    field_id: null,
    name: isZh ? '用户数量' : 'User Count',
    field_type: 'number',
  ...
  // Handlers
  const handleSelectOrgView = useCallback((viewId: number | 'all') => {
  ...
  const handleSelectGroup = useCallback((value: string | undefined) => {
    setGroupValue(value)
    setPage(1)
    router.replace(buildOrgsUrl(selectedViewId, value), { scroll: false })
  ...
  return (
    <div className="flex h-full">
                ...
                {orgViewsLoading ? (
                  ...
                  <>
                    <SidebarViewItem
                      ...
                      onClick={() => handleSelectOrgView('all')}
                    />
                    {orgViews?.map((v) => (
                      <SidebarViewItem
                        key={v.id}
                        ...
                        onClick={() => handleSelectOrgView(v.id)}
                      />
                    ))}
                  </>
                )}
      ...
      {/* Right: Main content — same layout as workspace users list (pen: title row + primary action, then search toolbar) */}
        ...
        <div className="flex items-center gap-3 px-5 pb-3.5 pt-3">
          <div className="relative w-full max-w-[240px]">
            ...
            <input
              ...
              placeholder={isZh ? '搜索组织...' : 'Search organizations...'}
              className={cn(
          ...
          {orgsLoading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3">
              <p className="text-sm text-muted-foreground">
                {hasActiveFilter
                  ? (isZh ? '无匹配条件的组织' : 'No organizations match the filter')
                  : (isZh ? '暂无组织数据' : 'No organization data')}
              </p>
              {hasActiveFilter && (
                <button
                  onClick={handleClearFilter}
...
function buildFieldLookup(fields: UnifiedField[], _locale: string): FieldValueLookup {
...
function getCellValue(
  ...
  // Special: user_count virtual column
